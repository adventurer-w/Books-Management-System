...
void BorrowRecord::loadRecords(){
    ...
    for(int i=0;i<10;i++){
        ...
        //btn_delete->setProperty("tb_ISBN",model->index(i,4,QModelIndex()).data().toString());
         connect(btn_delete,&QPushButton::clicked,this,&BorrowRecord::on_btn_delete_clicked);
        //将按钮加入表格中
        ui->tb->setIndexWidget(model->index(i,6),btn_delete);

    }
}
void BorrowRecord::on_btn_delete_clicked(){
    //删除记录

}

void BorrowRecord::on_btn_borrowInstruction_clicked()
{
    BorrowInstruction *instruction = new BorrowInstruction();
    instruction->show();
}

void BorrowRecord::on_line_search_returnPressed()
{
    //搜索框回车，逻辑和搜索按钮一样
}

void BorrowRecord::on_btn_search_clicked()
{
    //点击搜索按钮
}
bool BorrowRecord::loadQss(const QString &StyleSheetFile){
